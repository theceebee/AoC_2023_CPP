...
private:
    ...
    map<int, char> m_hand_map_flipped{};
...
Type Hand::getType() const
{
    ...
    bool has_joker = m_hand_map.count('J') == 1;
    ...
    else if (m_hand_map.size() == 3)
    {
        ...
        {
            // Could be four of a kind or full house.
            int joker_count = m_hand_map.at('J');
            ...
            {
                if (m_hand_map_flipped.count(3) == 1)  // 1 * Joker + 1 * x + 3 * y
                {
                    result = Type::four_of_a_kind;  // 1
                }
                else  // 1 * Joker + 2 * x + 2 * y
                {
                    result = Type::full_house;  // 2
                }
            }
